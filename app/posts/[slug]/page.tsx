...
export default async function PostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12 md:py-16">
      <Link
        href="/"
        className="smallcaps text-sm text-ink-muted hover:text-ink transition-colors"
      >
        ←&nbsp;&nbsp;Index
      </Link>
      ...
      <article className="mt-12 md:mt-16">
        <header className="mb-12 md:mb-16">
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="smallcaps text-sm text-ink-muted">Observation</span>
          </div>
        ...
        {post.coverImage && (
          <figure className="mb-16">
            ...
            />
            {post.painting && (
              <figcaption className="mt-4 text-center text-base text-ink-muted italic">
